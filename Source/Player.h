...
struct Player {
	...
	string note = "";
	int supp = 0, hs = 0;
	int mod = 0, dev = 0, chuher = 0, m_h = 0, admin = 0, staff = 0;
	int wls = 0;
	int csn = -1;
	bool IsTransformUbi = false;
	bool IsTransformDragon = false;
	bool IsTransformSpace = false;
...
inline bool isHere(ENetPeer* peer, ENetPeer* peer2) {
	return static_cast<Player*>(peer->data)->world == static_cast<Player*>(peer2->data)->world;
}
/*inline void Players::showWrong(ENetPeer* peer, string itemFind, string listFull) {
