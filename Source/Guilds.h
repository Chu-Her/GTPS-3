...
vector<Guild> guilds{};
uint32_t guild_create_gem_requirement = 100000; // 2x maziau nei real gt
				...
				if (member_search->display_name == "") {
					member_search->display_name = "`o" + member_search->member_name;
						...
						if (ifs.is_open()) {
							...
							if (j["admin"] == 1) member_search->display_name = "`6@" + member_search->member_name;
							if (j["staff"] == 1) member_search->display_name = "`8@" + member_search->member_name;
							if (j["dev"] == 1) member_search->display_name = "`9@" + member_search->member_name;
						}
				}
					...
					if (search_member->display_name == "") {
						search_member->display_name = "`o" + search_member->member_name;
							...
							if (ifs.is_open()) {
								json j;
								ifs >> j;
								if (j["mod"] == 1) search_member->display_name = "`#@" + search_member->member_name;
								if (j["admin"] == 1) search_member->display_name = "`6@" + search_member->member_name;
								if (j["staff"] == 1) search_member->display_name = "`8@" + search_member->member_name;
								if (j["dev"] == 1) search_member->display_name = "`9@" + search_member->member_name;
							}
					}
