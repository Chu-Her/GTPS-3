					...
					case 2:
					{
						...
						if (pInfo(peer)->trading_with == -1) {
							if (cch == "action|dialog_return\ndialog_name|\nbuttonClicked|newbie\n\n") {
								...
								if (pInfo(peer)->mod == 1 || pInfo(peer)->dev == 1 || pInfo(peer)->admin == 1 || pInfo(peer)->staff == 1) {
							...
							if (pInfo(peer)->offergems <= 0) break;
								...
								if (pInfo(peer)->gtwl >= 1500) {
									...
									p.CreatePacket(peer);
									pInfo(peer)->name_color = (pInfo(peer)->dev == 1 ? "`9@" : (pInfo(peer)->tmod == 1) ? "`#@" : (pInfo(peer)->admin == 1) ? "`6@" : (pInfo(peer)->staff == 1) ? "`8@" : "`0");
									for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
										if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL) continue;
										gamepacket_t p;
										p.Insert("OnConsoleMessage");
							...
							break;
								...
								if (pInfo(peer)->world == "" && pInfo(peer)->enter_game == 1) {
									if (pInfo(peer)->tankIDName.empty()) {
										...
										pInfo(peer)->name_color = (pInfo(peer)->dev == 1 ? "`9@" : (pInfo(peer)->tmod == 1) ? "`#@" : (pInfo(peer)->admin == 1) ? "`6@" : (pInfo(peer)->staff == 1) ? "`8@" : "`0");
										string thetag = (pInfo(peer)->mod || pInfo(peer)->admin || pInfo(peer)->staff || pInfo(peer)->dev ? "@" : "");
										...
										int on_ = 0, t_ = 0;
										//if  pInfo(peer)->superdev = 1, pInfo(peer)->dev = 1, pInfo(peer)->mod = 1, pInfo(peer)->gems = 999999, pInfo(peer)->gtwl = 99999;
										...
										}
										{
											...
											p.Insert(151749376);
											p.Insert("(wl)|ā|0&(yes)|Ă|0&(no)|ă|0&(love)|Ą|0&(oops)|ą|0&(shy)|Ć|0&(wink)|ć|0&(tongue)|Ĉ|1&(agree)|ĉ|0&(sleep)|Ċ|0&(punch)|ċ|0&(music)|Č|0&(build)|č|0&(megaphone)|Ď|0&(sigh)|ď|1&(mad)|Đ|1&(wow)|đ|1&(dance)|Ē|0&(see-no-evil)|ē|0&(bheart)|Ĕ|0&(heart)|ĕ|0&(grow)|Ė|0&(gems)|ė|0&(kiss)|Ę|0&(gtoken)|ę|0&(lol)|Ě|0&(smile)|Ā|1&(cool)|Ĝ|0&(cry)|ĝ|0&(vend)|Ğ|0&(bunny)|ě|0&(cactus)|ğ|0&(pine)|Ĥ|0&(peace)|ģ|0&(terror)|ġ|0&(troll)|Ġ|0&(evil)|Ģ|0&(fireworks)|Ħ|0&(football)|ĥ|0&(alien)|ħ|0&(party)|Ĩ|0&(pizza)|ĩ|0&(clap)|Ī|0&(song)|ī|0&(ghost)|Ĭ|0&(nuke)|ĭ|0&(halo)|Į|0&(turkey)|į|0&(gift)|İ|0&(cake)|ı|0&(heartarrow)|Ĳ|0&(lucky)|ĳ|0&(shamrock)|Ĵ|0&(grin)|ĵ|0&(ill)|Ķ|0&(eyes)|ķ|0&(weary)|ĸ|0&");
											p.CreatePacket(peer);
										}
										/*
										{
									...
									//	p5.Insert("OnConsoleMessage"), p5.Insert("`3It's Approximately `2Cinco `wDe `4Mayo!`` Party with your friends and smash Pinatas for prizes!``"), p5.CreatePacket(peer);
										if (pInfo(peer)->mod + pInfo(peer)->dev + pInfo(peer)->vip >= 1 + pInfo(peer)->admin + pInfo(peer)->staff) {
							...
							if (cch.find("action|dialog_return\ndialog_name|billboard_edit\nbillboard_toggle|") != string::npos) {
								...
								gamepacket_t p;
								p.Insert("OnDialogRequest");
								p.Insert("set_default_color|`o\nadd_label_with_icon|big|`9The Ringmaster``|left|1900|\nadd_smalltext|You are about to embark on a magical journey! It won't be easy, but the reward is one of the fine rings from my vast collection. You don't get to pick which one, mind you.|left|\nadd_smalltext|I've got a lot of rings sitting in my bag, with a lot of magical properties. If you want one, picked randomly at my discretion, you'll need to complete 10 little tasks for me.|left|\nadd_smalltext|Doesn't sound so bad does it?|left|\nadd_smalltext|Remember that I travel with the carnival, so if you don't get your quest done this time, be sure to catch me again next month!|left|\nadd_smalltext|There's one last thing you should know before you begin. You can quit your quest at any time, but be aware that if you do, you'll lose all your progress, and you'll have to pay another 10 Golden Tickets to start again!|left|\nadd_textbox|So... now that you've seen it all, are you truly prepared to hand over 10 Golden Tickets and embark on the Quest For The Ring?|left|\nend_dialog|ring_quest|No|Yes|\n");
							}
